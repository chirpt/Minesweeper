...
def get_selected_config():
    if sz.get() == "Small":
        ...
        selected_width = int(width_value.get())
        selected_height = int(height_value.get())
    if df.get() == "Easy":
        selected_mines = 0.1
        selected_lives = 0.02
    elif df.get() == "Medium":
        selected_mines = 0.15
        selected_lives = 0.015
    elif df.get() == "Difficult":
        selected_mines = 0.175
        selected_lives = 0.01
    elif df.get() == "Extreme":
        selected_mines = 0.20
        selected_lives = 0.005
    else:
        selected_mines = int(mines.get())
        selected_lives = int(lives.get())

    return selected_width, selected_height, selected_mines, selected_lives
def start_game():
    root.destroy()
